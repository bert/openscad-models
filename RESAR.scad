 ...
 * 
 * \brief Function to create a 3D model of a Through Hole Resistor Axial
 * Rectangular package (\c RESAR).
 *
...
// For a RESAR2540-1500X400X400 example uncomment the following line
//RES (1000, 25.40, 15.00, 4.00, 0.8);
...
(
  ...
  package_width,
    /*!< body width of the resistor. */
  package_height,
    /*!< body height of the resistor. */
  lead_diameter
...
)
{
  ...
  union()
  {
    ...
    color (RES_BODY)
    {
      translate ([0, 0, (package_height / 2)])
      {
        rotate ([0, 90, 0])
        {
          cube
    ...
    color (LEADS)
    {
      translate ([(-0.5 * (pitch_x - 2 * bend_radius)), 0, (package_height / 2)])
      {
        rotate ([0, 90, 0])
        {
          cylinder
